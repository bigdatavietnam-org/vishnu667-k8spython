...
def replace_from_dict(k8s_client, data, verbose=False, namespace='default',
    ...
    if api_exceptions:
        raise FailToCreateError1(api_exceptions)
...
def replace_from_yaml_single_item(
    ...
    if hasattr(k8s_api, "replace_namespaced_{0}".format(kind)):
        ...
        if "name" in yml_object["metadata"]:
            name = yml_object["metadata"]["namespace"]
            kwargs['name'] = name
        print("yml_object:\t"+str(yml_object))
        print(getattr(k8s_api, "replace_namespaced_{0}".format(kind)))
